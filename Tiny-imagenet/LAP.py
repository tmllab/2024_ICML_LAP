...
def main():
    ...
    if args.model == 'PreActResNet18':
        model = PreActResNet18(num_classes=200).cuda()
        proxy = PreActResNet18(num_classes=200).cuda()
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if args.delta_init == 'zero':
                delta = torch.zeros(args.batch_size, 3, 64, 64).cuda()
            elif args.delta_init == 'random':
                delta_ran = torch.zeros(args.batch_size, 3, 64, 64).cuda()
                if args.clamp:
    ...
    # Evaluation
    if args.model == 'PreActResNet18':
        model_test = PreActResNet18(num_classes=200).cuda()
    else:
        raise ValueError("Unknown model")
